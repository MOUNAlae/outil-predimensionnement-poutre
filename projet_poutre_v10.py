...
def calculer_moment_maximal(F, L):
    M_max = F * L
    return M_max
def calculer_contrainte_maximale(M_max, h, I):
    sigma_max = (M_max * h / 2) / I
    return sigma_max
def calculer_fleche_maximale(F, L, E, I):
    fleche_max = (F * L**3) / (3 * E * I)
    return fleche_max
...
def rechercher_hauteur_minimale(
    ...
    for hauteur_testee in hauteurs:
        I_test = calculer_moment_quadratique(
            b,
            hauteur_testee
        )
        ...
        sigma_test = calculer_contrainte_maximale(
            M_max_test,
            hauteur_testee,
            I_test
        )

        fleche_test = calculer_fleche_maximale(
            F,
            L,
            E,
            I_test
        ...
        facteur_securite_test = calculer_facteur_securite(
            Re,
            sigma_test
        )
        ...
        rigidite_test = verifier_rigidite(
            fleche_test,
            fleche_admissible
        )

        if resistance_test and rigidite_test:
            volume_test = calculer_volume(
                L,
                b,
                hauteur_testee
            )

            masse_test = calculer_masse(
                densite,
                volume_test
            )
...
def calculer_deformee(x, F, L, E, I):
    deformee = -(F * x**2 * (3 * L - x)) / (6 * E * I)
    return deformee
...
def tracer_deformee(x, deformee):
    ...
    plt.grid()
    plt.show()
def tracer_resultats_combines(x,effort_tranchant,moment_flechissant,deformee,nom_materiau):
    deformee_mm = deformee * 1000
    ...
    plt.show()
def exporter_resultats_csv(L,b,h,F,nom_materiau,E,Re,densite,volume,masse,I,M_max,sigma_max_MPa,fleche_max_mm,fleche_admissible_mm,facteur_securite,facteur_securite_minimal,resistance_respectee,rigidite_respectee,
hauteur_optimisee,
    sigma_optimisee,
    fleche_optimisee,
    ...
    with open(nom_fichier, "w", newline="", encoding="utf-8-sig") as fichier:
        ...
        ecrivain.writerow(["Moment quadratique", I, "m⁴"])
        ...
        ecrivain.writerow(
            ["Flèche admissible", fleche_admissible_mm, "mm"]
        )
        ecrivain.writerow(
        ...
        ecrivain.writerow(
            [
                ...
                "Oui" if resistance_respectee else "Non",
                ""
            ]
        )
        ...
        ecrivain.writerow(
            [
                "Rigidité respectée",
                "Oui" if rigidite_respectee else "Non",
                ""
            ]
        )
        ...
        ecrivain.writerow(
                ["Hauteur initiale", round(h * 1000, 1), "mm"]
            )

        ecrivain.writerow(
                [
                    "Hauteur optimisée",
                    round(hauteur_optimisee * 1000, 1),
                    "mm"
                ]
            )

        ecrivain.writerow(
                ["Masse initiale", round(masse, 2), "kg"]
            )

        ecrivain.writerow(
                [
                    "Masse optimisée",
                    round(masse_optimisee, 2),
                    "kg"
                ]
            )
        ...
        ecrivain.writerow(
                [
                    "Contrainte optimisée",
                    round(sigma_optimisee / 1e6, 2),
                    "MPa"
                ]
            )
        ...
        ecrivain.writerow(
                [
                    "Facteur de sécurité optimisé",
                    round(facteur_securite_optimise, 2),
                    ""
                ]
            )
...
def main():
  ...
  try:
      ...
      if choix_materiau != 1 and choix_materiau != 2:
          ...
          I = calculer_moment_quadratique(b,h)
          print(f"Moment quadratique : {I:.6e} m^4")
          ...
          deformee = calculer_deformee(x, F, L, E, I)
          sigma_max = calculer_contrainte_maximale(M_max, h, I)
          sigma_max_MPa = sigma_max / 1e6
          fleche_aluminium = calculer_fleche_maximale(F, L, E_aluminium, I)
          fleche_acier = calculer_fleche_maximale(F, L, E_acier, I)

          fleche_aluminium_mm = fleche_aluminium * 1000
          fleche_acier_mm = fleche_acier * 1000
          facteur_securite_aluminium = calculer_facteur_securite(Re_aluminium, sigma_max)
          facteur_securite_acier = calculer_facteur_securite(Re_acier, sigma_max)

          print(f"Contrainte maximale : {sigma_max_MPa:.2f} MPa")
          fleche_max = calculer_fleche_maximale(F, L, E, I)
          ...
          print(f"Facteur de sécurité minimal demandé : {facteur_securite_minimal:.2f}" )
          ...
          resistance_respectee = verifier_resistance(facteur_securite,facteur_securite_minimal)
          ...
          if resistance_respectee and rigidite_respectee:
              print("Dimensionnement validé : les critères de résistance et de rigidité sont respectés.")
          else:
              print( "Dimensionnement non validé : au moins un critère n'est pas respecté.")
          ...
          if solution_optimisee is not None:
                ...
                print(
                    f"Contrainte correspondante : "
                    f"{sigma_optimisee / 1e6:.2f} MPa"
                )
                
                print(
                    f"Flèche correspondante : "
                ...
                print(
                    f"Masse correspondante : "
                    f"{masse_optimisee:.2f} kg"
                )
                print("\n--- COMPARAISON INITIAL / OPTIMISÉ ---")

                print(
                    f"Hauteur initiale : {h * 1000:.1f} mm"
                )
                ...
                print(
                    f"Réduction de masse : "
                    f"{reduction_masse:.2f} kg"
                )

                print(
                    f"Réduction relative : "
                    f"{pourcentage_reduction:.1f} %"
                )
                exporter_resultats_csv(L,b,h,F,nom_materiau,E,Re,densite,volume,masse,I,M_max,sigma_max_MPa,fleche_max_mm,fleche_admissible_mm,facteur_securite,facteur_securite_minimal,resistance_respectee,rigidite_respectee,
hauteur_optimisee,
                sigma_optimisee,
                fleche_optimisee,
                facteur_securite_optimise,
      ...
      if (
              solution_aluminium is not None
              and solution_acier is not None
          ):
          (
          ...
          print(
                  f"Hauteur minimale : "
                  f"{hauteur_optimisee_aluminium * 1000:.1f} mm"
              )
          print(
                  f"Masse optimisée : "
                  f"{masse_optimisee_aluminium:.2f} kg"
              )
          print(
                  f"Contrainte : "
                  f"{sigma_optimisee_aluminium / 1e6:.2f} MPa"
              )
          print(
                  f"Flèche : "
                  f"{fleche_optimisee_aluminium * 1000:.3f} mm"
              )
          print(
                  f"Facteur de sécurité : "
                  f"{facteur_securite_optimise_aluminium:.2f}"
              )

          print("\nAcier de construction :")
          print(
                  f"Hauteur minimale : "
                  f"{hauteur_optimisee_acier * 1000:.1f} mm"
              )
          print(
                  f"Masse optimisée : "
                  f"{masse_optimisee_acier:.2f} kg"
              )
          print(
                  f"Contrainte : "
                  f"{sigma_optimisee_acier / 1e6:.2f} MPa"
              )
          print(
                  f"Flèche : "
                  f"{fleche_optimisee_acier * 1000:.3f} mm"
              )
          print(
                  f"Facteur de sécurité : "
                  f"{facteur_securite_optimise_acier:.2f}"
              )
          print("\n--- AIDE AU CHOIX ---")
          ...
          if masse_optimisee_aluminium < masse_optimisee_acier:
                  ...
                  print(
                      "Les deux solutions optimisées ont la même masse."
                  )
          if (
                  hauteur_optimisee_aluminium
                  < hauteur_optimisee_acier
                  ...
                  print(
                      "Section de plus faible hauteur : "
                      "Aluminium 2024-T3."
                  )
                  ...
                  print(
                      "Les deux solutions ont la même hauteur."
                  )
      else:
              print(
      ...
      tracer_resultats_combines(x,effort_tranchant,moment_flechissant,deformee,nom_materiau)
...
def lancer_interface():
    ...
    sous_titre = ttk.Label(
        fenetre,
        text=(
            "Calcul de résistance, rigidité, masse "
            "et recherche d'une hauteur minimale"
        ),
        font=("Arial", 10)
    )
    ...
    ttk.Label(
        cadre_saisie,
        text="Facteur de sécurité minimal :"
    ).grid(
        row=4,
        column=0,
        sticky="w",
    ...
    ttk.Entry(
        cadre_saisie,
        textvariable=facteur_minimal_var,
        width=20
    ).grid(
        row=4,
        column=1,
    ...
    barre_defilement.config(
        command=texte_resultats.yview
    )
    barre_defilement.pack(
        side="right",
        fill="y"
    )
    ...
    texte_resultats.insert(
        "1.0",
        "Renseignez les données puis cliquez sur "
    ...
    def calculer_interface():
        try:
            ...
            (
                ...
                Re,
                densite
            ) = selectionner_materiau(choix_materiau)
            ...
            sigma_max = calculer_contrainte_maximale(
                M_max,
                h,
                I
            )
            ...
            fleche_max = calculer_fleche_maximale(
                F,
                L,
                E,
                I
            ...
            rigidite_respectee = verifier_rigidite(
                fleche_max,
                fleche_admissible
            )
            ...
            if resistance_respectee and rigidite_respectee:
                resultat += (
                    "Conclusion : dimensionnement initial validé.\n"
                )
            else:
                resultat += (
                    "Conclusion : dimensionnement initial "
                    "non validé.\n"
                )
            if solution_optimisee is not None:
                (
         ...
         messagebox.showerror(
                "Erreur de saisie",
                "Veuillez entrer uniquement des valeurs numériques."
            )
    bouton_calculer = ttk.Button(
        fenetre,
        text="Calculer et optimiser",
        command=calculer_interface
    )

    bouton_calculer.pack(
        before=cadre_resultats,
        pady=20
    )
